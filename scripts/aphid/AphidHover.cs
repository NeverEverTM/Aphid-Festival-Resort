...
public partial class AphidHover : InteractableArea2D
{
    ...
    {
        if (FreeCameraManager.Enabled && !CanvasManager.IsTakingScreenshot && !CanvasManager.Menus.IsActive)
        {
    ...
    {
        if (!mouseIshovering || !FreeCameraManager.Enabled || CanvasManager.Menus.IsActive)
            return;
