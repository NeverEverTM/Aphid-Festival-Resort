...
public partial class DialogManager : Control
{
	...
	[Export] private RichTextLabel dialogText, dialogName;
	...
	public async Task OpenDialogBox(string _dialog_key, string _id = null, AudioStream _voice = null)
	{
		...
		if (_id != null)
		{
			dialogName.GetParent<Control>().Show();
			dialogName.Text = _id.ToPascalCase();
		}
		else
	...
	private void RegisterDialogCommand(int _startIndex)
	{
		...
		if (Commands.TryGetValue(_command, out IDialogCommand value))
		{
			string _textToInsert = value.Execute(_args) ?? string.Empty;
    ...
    public class PronounsPossesiveES : IDialogCommand
    {
        public string Execute(string[] _args)
        {
			// accounting for both EN and ES pronouns anyways in case they switched languages
			if (Player.Data.Pronouns[0].Equals("el", StringComparison.OrdinalIgnoreCase)
				|| Player.Data.Pronouns[0].Equals("he", StringComparison.OrdinalIgnoreCase))
				return "o";
			if (Player.Data.Pronouns[0].Equals("ella", StringComparison.OrdinalIgnoreCase)
				|| Player.Data.Pronouns[0].Equals("she", StringComparison.OrdinalIgnoreCase))
		   		return "a";
			else
				return "e";
        }
	...
	public class PronounsPossesiveEN : IDialogCommand
    {
		public string Execute(string[] _args)
		{
			...
			if (Player.Data.Pronouns[0].Equals("ella", StringComparison.OrdinalIgnoreCase)
				|| Player.Data.Pronouns[0].Equals("she", StringComparison.OrdinalIgnoreCase))
				return "hers";
			else if (Player.Data.Pronouns[0].Equals("elle", StringComparison.OrdinalIgnoreCase)
				|| Player.Data.Pronouns[0].Equals("they", StringComparison.OrdinalIgnoreCase))
				return "e";
			else
				return "x";
        }
